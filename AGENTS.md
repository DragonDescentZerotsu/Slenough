...
3. 默认 runtime 实验 profile：60 秒 epoch、`motionSampleHz = 0.2`。
...
6. WatchConnectivity summary sync，默认每 15 个 epoch batch 一次，约 15 分钟同步一次，含 queued delivery fallback。
...
- `WatchConnectivitySender.swift`：Watch -> iPhone 低频 summary 同步。默认缓存 epoch summaries，每 15 个 epoch batch 发送，当前约每 15 分钟一次；实时 reachable 时用 `sendMessage`，同时用 `transferUserInfo` 保底。
...
- `Phase2Models.swift`：`SleepState`、`ProbeSessionState`、`SleepRuleConfig`、`EpochSummary`、`WatchEpoch`、`WatchEventRecord` 等共享模型。当前默认 `epochSeconds = 60`、`motionSampleHz = 0.2`、`syncEveryNEpochs = 15`。
- `SleepRuleEngine.swift`：第一版可解释 motion-first 规则模型。默认 settling 10 分钟，连续 10 个低 motion epoch 才判定 asleep，高 motion/burst 判定 awake/restless。只累计 `predictedState == asleep` 的 epoch。
